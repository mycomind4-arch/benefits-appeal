/**
 * LLM Bridge — adapts @mailmypdf/ai's shared router to benefits-appeal's
 * existing LLM service API (identical pattern to appeal-mail).
 */
import {
  routeLLMRequest,
  getLLMConfig,
...
export function getProviderLabel(provider: LLMProvider): string {
  const labels: Record<LLMProvider, string> = { gemini: 'Google Gemini', claude: 'Anthropic Claude', openai: 'OpenAI GPT-4o' };
